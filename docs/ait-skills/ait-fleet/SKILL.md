...
name: ait-fleet
description: >
  Multi-repo orchestration for `ait`. Shared across agents — Codex, Claude,
  OpenCode, Droid, Pi. Use when a project spans more than one git repository
  (e.g. separate frontend and backend repos) and you need to plan, track, or
  coordinate work across them. Triggers on mentions of "ait fleet", "across
  repos", "multi-repo ait", "frontend and backend ait", or any request to set
  up or use ait across more than one repo.
version: "1.0.0"
---
...
This skill is part of the single shared `ait` contract. It adds a
multi-repo convention on top of the core `ait` skill; read that one first.
...
`ait init` is a Lead-only command in the role matrix. Don't run it without
explicit confirmation — it's the user's repo and `init` writes the prefix.
A Peer that needs a new member DB should surface the need to the Lead
instead of running `init` itself.
...
Then write `~/.config/ait-fleet/<name>.json` with the confirmed JSON.
...
`close --cascade` is a Lead action. When all member epics are done, close
the initiative in the primary repo (Lead).
