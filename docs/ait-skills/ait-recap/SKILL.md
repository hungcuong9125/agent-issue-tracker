...
name: ait-recap
description: >
  Generate a friendly markdown recap of recent `ait` activity for the current
  project, or scan a directory of projects. Shared across agents — Codex,
  Claude, OpenCode, Droid, Pi. Use when the user asks "what have I been doing
  this week?", "what shipped recently?", wants an aide-mémoire before a status
  update with a manager, or asks for a summary of recent activity across their
  projects.
version: "1.0.0"
---
...
Every command this skill runs is read-only (`config`, `log --long`,
`list --all --long`), so it is safe for every seat: Lead, Peer, and
Supervisor. You should never need to claim or write anything.
...
Find candidate databases by scanning for the tracker files:
...
- The canonical `ait` claim identity is `PASEO_AGENT_ID` — but this skill is
  **read-only**, so you shouldn't need to claim anything at all.
