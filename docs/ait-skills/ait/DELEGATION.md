...
`ait export` is read-only, so it is safe for every seat: Lead, Peer, and
Supervisor can all produce briefings.
...
Markdown and update the central tracker. Reconcile actions follow the role
matrix in `SKILL.md`:

- **All tasks completed**: use cascade close on the epic — Lead only
  ```bash
  ait close <epic-id> --cascade
  ```
- **Partial completion**: close individual tasks that were ticked off —
  Peer may close its own task; Lead may close any task
  ```bash
  ait close <task-id>
  ```
- **Notes worth preserving**: add them to the tracker (Peer and Lead)
  ```bash
...
- If the sub-agent creates new work (discovered tasks, follow-ups), capture
  those as new issues in the tracker during reconciliation — creation is a
  Lead action in the role matrix.
